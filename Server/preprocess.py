...
class Loader:
    ...
    def load(self, file_path):
        signal = librosa.load(file_path,
                              ...
                              mono = self.mono)[0]  #Returns Signal as well as Rate. We only want Signal.
        return signal 
...
class LogSpectogramExtractor:
    """LogSpectogramExtractor extracts log spectograms (in db) from a
    time-series signal.""" 
...
class PreprocessingPipeline:
    ...
    Storing the min max values for all the log spectograms.
    """ 
    ...
    def process(self, audio_files_dir):
        for root, _, files in os.walk(audio_files_dir):
            for file in files:
                ...
                print(f"Processed fie {file_path}")
